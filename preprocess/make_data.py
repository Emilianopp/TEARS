...
def split_train_test_proportion(data, test_prop=0.2):
    ...
    promt_dataset = pd.read_csv(
        f'./data_preprocessed/{args.data_name }/prompt_set_timestamped.csv')
...
if __name__ == '__main__':
    ...
    vad_plays_tr.to_csv(os.path.join(
        DATA_DIR, 'validation_tr_.csv'), index=False)
    vad_plays_te.to_csv(os.path.join(
        DATA_DIR, 'validation_te_.csv'), index=False)
