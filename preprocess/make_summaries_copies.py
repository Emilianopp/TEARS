...
parser.add_argument("--len", type=int, default=200, help="Maximum number of tokens for the response")
parser.add_argument("--v", type=int, default=200, help="Maximum number of tokens for the response")
...
for i, (idx, user_prompt) in (pbar := tqdm(enumerate(prompts_to_process[0].items()))):
    ...
    with open(f'./saved_user_summary/{args.data_name}/user_summary_gpt4_sum_length_{args.len}_{args.v}.json', 'w') as fp:
        #make keys int 
