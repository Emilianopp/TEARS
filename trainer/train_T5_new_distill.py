...
from model.MF import MultiVAE, sentenceT5Classification,sentenceT5ClassificationFrozen,MultiDAE
...
def train_fun(rank,world_size):
    try:
        ...
        if args.embedding_module == 't5_classification':
            ...
            model = sentenceT5ClassificationFrozen.from_pretrained('t5-large', num_labels=n_items, classifier_dropout = args.dropout)
            for name, param in model.named_parameters():
                if 'classification_head' not in name:
                    param.requires_grad = False
        elif args.embedding_module == 't5_distill': 
            model = sentenceT5Classification.from_pretrained('t5-large', num_labels=n_items, classifier_dropout = args.dropout)
            lora_config = LoraConfig(task_type=TaskType.SEQ_CLS, r=args.lora_r, lora_alpha=args.lora_alpha, lora_dropout=args.dropout,
                                    target_modules=["q", "v"],
                                    modules_to_save=['classification_head'])
            ...
            teacher = MultiVAE(p_dims,dropout = args.dropout)
            teacher_path = args.save_teacher_path + f'model_all_DAE_{args.data_name}.pt'
        ...
        train_fun = train_distill if args.embedding_module == 't5_distill' else train_fineTuning 
        val_recall = 0
