...
def train_fun(rank,world_size):
    ...
    prompts,rec_dataloader,num_movies,val_dataloader,test_dataloader,val_data_tr,test_data_tr,non_bin_dataloader,tokenizer= load_data_items_as_tokens(args,tokenizer,rank,world_size)
    print(f"{prompts[1]=}")
    model_name = f"{args.embedding_module}-small"
    
    if args.embedding_module == 't5_classification':
        model = sentenceT5Classification.from_pretrained('t5-small', num_labels=num_movies, classifier_dropout = args.dropout)
