...
def get_embeddings(model, dataloader, rank, world_size, num_movies, tokenizer, save_path, save_name, save=True):
    if os.path.exists(os.path.join(save_path, save_name)):
        embeddings = np.load(os.path.join(save_path, save_name))
    else:
        print(f"{os.path.join(save_path, save_name)=}")
        model.eval()
        embeddings = {}
        with torch.no_grad():
            for batch in dataloader:
                ...
                for i in range(len(idx)):
                    embeddings[idx[i].item()] = batch_embeddings[i]
        ...
        if rank == 0 and save:
            ...
            np.save(os.path.join(save_path, save_name), embeddings)
    ...
    for b, (items,augmented_items) in enumerate(zip(rec_dataloader,augmented_dataloader)):
        ...
        with torch.cuda.amp.autocast():
            ...
            if precompute_embeddings is not None: 
                hidden_states = precompute_embeddings[items['idx'].cpu().numpy()]
                hidden_states = torch.tensor(hidden_states).to(rank)
                movie_emb = model.classifier_forward(hidden_states)
            else: 
