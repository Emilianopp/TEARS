...
class GenreModel(nn.Module):
    ...
    def generate_recommendations(self,data_tensor,topk,alpha = .5,neg = False,mask_genre = None):
        ...
        return ranked_logts.indices.flatten().tolist() 
    def generate_recommendations_batch(self, data_tensor, topk, fav_genre=None,mask_genre=None,proportional = False):
        ...
        if mask_genre is not None :
            ...
            #here proportional refers to calculating \Delta@k
            if  proportional: 
            ...
            #this calculates \Delta@k_\uparrow
            else: 
                for i,(mask,fav) in enumerate(zip(mask_indices,fav_genre_indecies)):
...
def get_model(args, num_movies):
    ...
    elif args.embedding_module == 'GenreTEARSRecVAE':
...
def get_GenreTEARS(args,num_movies):
    ...
    model = GenreModel(num_genres=num_genres,
