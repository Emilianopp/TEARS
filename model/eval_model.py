...
class LargeScaleEvaluator(nn.Module):
    ...
    def getDelta(self, s1, s2, labels, topk, genre_1, genre_2, rank, neg=False, max_delta=False):
        if self.args.mask_control_labels:
            labels = torch.zeros_like(labels)

        if isinstance(self.model, DistributedDataParallel):
            generate_recs = self.model.module.generate_recommendations
        else:
            generate_recs = self.model.generate_recommendations

        if self.args.embedding_module not in ['RecVAEGenreVAE', 'GenreTEARS']:
            topk1 = generate_recs(s1, self.tokenizer, labels,
                                  topk, rank, alpha=self.alpha, neg=neg)
            topk2 = generate_recs(s2, self.tokenizer, labels, topk, rank,
                                  alpha=self.alpha2 if self.alpha2 else self.alpha, neg=neg)
        else:
            topk1 = generate_recs(
                data_tensor=labels, topk=topk, rank=rank, alpha=self.alpha, neg=neg)
            topk2 = generate_recs(data_tensor=labels, mask_genre=genre_2, topk=topk,
                                  rank=rank, alpha=self.alpha2 if self.alpha2 else self.alpha, neg=neg)
        ...
        if max_delta:
            change_down = self.genrewise_ndcg(
                movie_genres1, genre_1, min_k=0, max_k=topk) - 0
            change_up = self.genrewise_ndcg(
                movie_genres1, genre_2, min_k=0, max_k=topk) - 1
        else:
            change_down = self.genrewise_ndcg(
                movie_genres1, genre_1, min_k=0, max_k=topk) - self.genrewise_ndcg(movie_genres2, genre_1, min_k=0, max_k=topk)
            change_up = self.genrewise_ndcg(movie_genres1, genre_2, min_k=0, max_k=topk) - \
                self.genrewise_ndcg(movie_genres2, genre_2,
                                    min_k=0, max_k=topk)
    ...
    def promptGPT(self, k_sample, prompts, labels, seed=2024, split='val'):
        ...
        self.genre_list = [x.lower().replace('-', ' ')
                           for x in self.genre_list]
        c = 0
        # set up a .env file on the root directory with the openai api key
        ...
        for k in (pbar := tqdm(k_sample)):
            ...
            if favorite_genre not in self.genre_list or least_favorite_genre not in self.genre_list:
                ...
                pbar.set_description(
                    f"pass {c} {favorite_genre} {least_favorite_genre}")
        ...
        for b in (pbar := tqdm(dataloader, desc='Controlability')):
            ...
            for uid, label in zip(uids, labels):
                if uid in self.df['keys'].values:
                    ...
                    down, up, movies1, movies2 = self.getDelta(
                        prompts[uid], gpt_output, label, topk, genre1, genre2, rank=rank, max_delta=max_delta)

                    self.delta_up.append(up)
                    self.delta_down.append(down)

            print(
                f"average ndcgs up {np.mean(self.delta_up)} average ndcg down {np.mean(self.delta_down)}")
            pbar.set_description(
                f"average ndcgs up {np.mean(self.delta_up)} average ndcg down {np.mean(self.delta_down)}")
        ...
        for b in (pbar := tqdm(dataloader, desc='Controlability')):
            ...
            for uid, label in zip(uids, labels):
                for genre in genres:
                    ...
                    #For guided recommendtaion we only look at the up direction
                    down, up, movies1, movies2 = self.getDelta(
                        prompts[uid], '', label, topk, genre, 'Action', rank=rank, max_delta=False)
                ...
                pbar.set_description(
                    f"average ndcgs up {np.median(self.delta_up)} average ndcg down {np.mean(self.delta_down)}")
        if return_arr:
