...
class SteepHC(nn.Module):
    def __init__(self, opt,editor):
        ...
        self.plm =AutoModelForCausalLM.from_pretrained('EleutherAI/gpt-neo-1.3B')
        self.plm.eval()
        self.plm.to(device)
    ...
    def scorer(self, input_news,ref_oris,state_vec=None,style_scorer = None,model = None,user_id = None,movie_id=None):
        
        fluency_scores=self.fluency_scorer(input_news) # input-news, ref_oris-->["I like you"]
        style_scores,style_labels=self.style_scorer(input_news) if style_scorer is None else style_scorer(input_news,movie_id=movie_id,user_id=user_id,model=model) # input-news, ref_oris-->["I like you"
    ...
    def acceptance_prob(self, input_news, input_olds,ref_oris,state_vec,style_scorer = None,model = None,movie_id = None,user_id=None):
        ref_old_score, preds_old = self.scorer(input_olds, ref_oris, state_vec,style_scorer,model = model,user_id = user_id,movie_id = movie_id)


        pos_old = torch.where(preds_old== movie_id)

        ref_new_scores, preds_new=self.scorer(input_news,ref_oris,state_vec,style_scorer,user_id= user_id,movie_id = movie_id)
        pos_new = torch.where(preds_new == movie_id)
        
        ref_new_score=ref_new_scores
        ...
        if movie_id in preds_new and (  pos_new[0].item() <= 9  ):
            new_style_labels = 1
        ...
        if  not torch.equal(preds_new, preds_old): 
            diff_indices = torch.nonzero(preds_new != preds_old)

            
            
            
        return ref_old_score,ref_new_score,new_style_labels, accept_hat,preds_new,preds_old


    def acceptance_prob_lower(self, input_news, input_olds,ref_oris,state_vec,style_scorer = None,model = None,movie_id = None,user_id=None):
        ref_old_score, preds_old = self.scorer(input_olds, ref_oris, state_vec,style_scorer,model = model,user_id = user_id,movie_id = movie_id)
