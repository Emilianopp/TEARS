...
class RobertaEditor(nn.Module):
    def __init__(self, opt):
        ...
        self.model = RobertaForMaskedLM.from_pretrained(self.model_dir, return_dict=True,max_position_embeddings =1024,ignore_mismatched_sizes =True).to(device)
    ...
    def generate(self, input_texts, max_len):
        ...
        for idx,rbt_sent in enumerate(rbt_sent_list):
            ...
            mask_token_index = (ids==self.tokenizer.mask_token_id).int().max(dim=1).indices
        ...
        random_sample = random.sample(total_sent_list[0],1)
